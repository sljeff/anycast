...
> **2026-09-22 进度**：自动化资产全部建成（`native` 分支）——RSS 语料 8 桶（真实源 22 个 + 构造桶）、API fixtures 10 端点、DB 7 桶由真实建表/模型代码生成、音频 8 类、OPML 4 份、封面 8 张、G1–G16 golden 全量导出、一键脚本 `tool/m0/regen_all.sh`、目录说明 `test/fixtures/README.md`。**待人工**：真机容器采集验证、真机数据样本替换（当前 DB 样本为"真实语料+真实代码"合成）、golden 人工抽查 ≥3 组、TF 基线 build、招募用户。生成细节见 test/fixtures/README.md；基线文档已按实测修正三处（01 §1.2 缓存列名、05 §1.5 G1/G13）。
>
...
- [ ] 验证容器采集可行性：真机确认 Xcode Download/Replace Container 对 dev 签名可用、对现网 App Store 版不可用（05 §1.1 修正规程）【待人工/真机】
...
- [ ] golden 复核抽查 ≥3 组（人工确认导出值与旧版运行表现一致）【待人工】
- [ ] 发最后一个 Flutter 版 TestFlight build（此后它就是覆盖安装测试的"旧版"与双设备对照机）【待人工/ASC】
- [ ] 招募 2–3 位重度用户进 TF 内部组（决策 D8）【待人工】
