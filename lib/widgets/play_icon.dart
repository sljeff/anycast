...
class PlayIcon extends GetView<PlayerController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      if (isLoading) {
        if (resolvedColor.computeLuminance() > .5) {
      ...
      if (isPlaying) {
        return Iconify(Ic.round_pause, size: size, color: resolvedColor);
      }
      return Iconify(Ic.round_play_arrow, size: size, color: resolvedColor);
    });
  }
}
